...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams LEFT JOIN games g1 ON teams.team_id = g1.winner_id LEFT JOIN games g2 ON teams.team_id = g2.opponent_id WHERE (g1.year=2014 AND g1.round='Eighth-Final') OR (g2.year=2014 AND g2.round='Eighth-Final')")"
...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams LEFT JOIN games g1 ON teams.team_id = g1.winner_id LEFT JOIN games g2 ON teams.team_id = g2.opponent_id WHERE name LIKE 'Co%'")"
